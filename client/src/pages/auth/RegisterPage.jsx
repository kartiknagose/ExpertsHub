// RegisterPage — uses Clerk <SignUp> when Clerk is configured (email verification
// is handled by Clerk), falls back to the custom form for the legacy path.
      ...
      {isSuccess ? (
        ...
        <Motion.div key="form" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Input label="Email Address" type="email" placeholder="you@example.com" icon={Mail} error={errors.email?.message} {...register('email', { onChange: () => { setServerError(''); clearError(); } })} />
            <Input label="Mobile Number" type="tel" placeholder="9876543210" icon={Phone} error={errors.mobile?.message} hint="10-digit mobile number without country code" {...register('mobile')} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input label="Password" type="password" placeholder="Min 8 chars" icon={Lock} error={errors.password?.message} {...register('password')} />
              <Input label="Confirm Password" type="password" placeholder="Repeat password" icon={Lock} error={errors.confirmPassword?.message} {...register('confirmPassword')} />
            </div>
            ...
            <Button type="submit" fullWidth size="lg" variant="gradient" loading={isSubmitting} className="h-14 text-base font-bold mt-2 rounded-2xl shadow-xl shadow-brand-500/20">
            ...
            <div className="text-center border-t pt-5 border-neutral-200 dark:border-dark-700">
              <p className="text-sm text-neutral-500 dark:text-neutral-400">
                ...
                <button type="button" onClick={() => navigate('/login')} className="font-bold text-brand-600 hover:text-brand-500 dark:text-brand-400 inline-flex items-center gap-1 transition-colors">
            ...
            <div className="pt-2">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-neutral-400 mb-3">What you get:</p>
...
export function RegisterPage() {
  ...
  const roleTitle = initialRole === 'worker' ? 'Join as a Professional' : 'Join as a Customer';
  const roleSubtitle = initialRole === 'worker'
  ...
  if (isClerkEnabled) {
    ...
    // 1. Clerk's <SignUp> handles email + password + email verification.
    // 2. After sign-up Clerk redirects to /onboarding where we collect mobile & role.
    return (
      <AuthLayout title={roleTitle} subtitle={roleSubtitle}>
        <SignUp
          ...
          path="/register"
          signInUrl="/login"
          fallbackRedirectUrl={`/onboarding?role=${initialRole}`}
          appearance={{
            elements: {
              rootBox: 'w-full',
